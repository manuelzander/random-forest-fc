...
- Add a `Next Game` card that highlights the next scheduled match with:
  ...
  - A direct `Sign Up` action for the live current season
- Add a few compact season insight cards, likely:
  - `Total Players`
  - `Games Played`
  - `Table Leader` using the existing ranking data
...
## Season behavior
- Current/live season: show actionable `Next Game` with sign-up CTA.
