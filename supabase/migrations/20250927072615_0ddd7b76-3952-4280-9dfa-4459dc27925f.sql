-- Fix the get_player_stats function to resolve the "argument of CASE/WHEN must not return a set" error
  ...
  LEFT JOIN (
    ...
    FROM (
      ...
      SELECT 
        ...
        CASE WHEN mvp_player = ANY(team1_players) THEN true ELSE false END as is_mvp
      FROM games
      ...
      SELECT 
        ...
        CASE WHEN mvp_player = ANY(team2_players) THEN true ELSE false END as is_mvp
      FROM games
    ) game_results
    GROUP BY player_id
